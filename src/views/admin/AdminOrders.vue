...
import { 
  ...
  Calendar, CreditCard, Coffee, Filter 
...
const selectedOrder = ref(null) // For the details modal
  ...
  <div class="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500 pb-10">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
      <div>
        ...
        <p class="text-xs text-slate-500 mt-1">View and manage all customer orders.</p>
      </div>
        ...
        <table class="w-full text-left border-collapse">
          ...
          <tbody class="divide-y divide-slate-50">
            <tr v-if="loading" class="animate-pulse">
              <td colspan="6" class="px-6 py-8 text-center text-xs text-slate-400">Loading orders...</td>
            </tr>
            
            <tr v-else-if="paginatedOrders.length === 0">
              <td colspan="6" class="px-6 py-8 text-center text-xs text-slate-400">No orders found.</td>
            </tr>
            ...
            >
              <td class="px-6 py-4 text-xs font-mono text-slate-500 font-medium">#{{ order.id }}</td>
              ...
              <td class="px-6 py-4 text-right">
                <span class="text-xs font-semibold text-slate-900">{{ formatCurrency(order.total_amount) }}</span>
              </td>
              
              <td class="px-6 py-4 text-center">
                <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-[10px] font-medium bg-green-50 text-green-700 border border-green-100">
                  Completed
                </span>
              </td>
    ...
    <div v-if="selectedOrder" class="fixed inset-0 z-50 flex items-center justify-center px-4">
      ...
      <div class="relative bg-white w-full max-w-lg rounded-2xl shadow-2xl overflow-hidden animate-in zoom-in-95 duration-200">
        ...
        <div class="p-6 max-h-[60vh] overflow-y-auto">
          <div class="flex items-center justify-between mb-6 pb-6 border-b border-slate-50 border-dashed">
            <div class="flex items-center gap-3">
              <div class="p-2 bg-slate-100 rounded-full"><Calendar class="w-4 h-4 text-slate-500" /></div>
              <div>
                <p class="text-[10px] text-slate-400 uppercase tracking-wider font-semibold">Date</p>
                <p class="text-xs font-medium text-slate-900">{{ formatDate(selectedOrder.created_at) }}</p>
              </div>
            </div>
             <div class="flex items-center gap-3 text-right">
              <div>
                <p class="text-[10px] text-slate-400 uppercase tracking-wider font-semibold">Status</p>
                <span class="text-xs font-semibold text-green-600">Completed</span>
              </div>
              <div class="p-2 bg-green-50 rounded-full"><Coffee class="w-4 h-4 text-green-600" /></div>
            </div>
          </div>
            ...
            >
              ...
              <div class="flex-1 min-w-0">
                ...
                <div class="flex flex-wrap gap-2 mt-2">
                  <span v-if="item.modifiers?.sugar" class="inline-flex text-[10px] px-1.5 py-0.5 bg-white border border-slate-200 rounded text-slate-500">
                    Sugar: {{ item.modifiers.sugar }}
                  </span>
                  <span v-if="item.modifiers?.ice" class="inline-flex text-[10px] px-1.5 py-0.5 bg-white border border-slate-200 rounded text-slate-500">
        ...
        <div class="px-6 py-4 bg-slate-50 border-t border-slate-100 flex justify-between items-center">
          <span class="text-xs font-semibold text-slate-500 uppercase tracking-wider">Total Amount</span>
          <span class="text-xl font-semibold text-slate-900">{{ formatCurrency(selectedOrder.total_amount) }}</span>
        </div>
      </div>
    </div>
