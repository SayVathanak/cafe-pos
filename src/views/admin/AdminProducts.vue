...
import { 
  ...
  Edit2, Trash2, PackageOpen, Star, ChevronRight,
  MoreVertical, Filter
  ...
  <div class="mx-auto min-h-screen bg-slate-50 pb-24 lg:pb-12">
    ...
    <header class="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
      <div>
        ...
        <p class="text-sm text-slate-500 mt-1">Manage your catalogue, pricing, and availability.</p>
      </div>
      
      <div class="hidden md:flex items-center gap-3">
        <div class="bg-white p-1 rounded-lg border border-slate-200 shadow-sm flex items-center">
          <button @click="viewMode = 'grid'" 
            :class="viewMode === 'grid' ? 'bg-indigo-50 text-indigo-600 shadow-sm' : 'text-slate-400 hover:text-slate-600'" 
            class="p-2 rounded-md transition-all duration-200">
            <LayoutGrid class="w-4 h-4" />
          </button>
          <div class="w-px h-4 bg-slate-100 mx-1"></div>
          <button @click="viewMode = 'list'" 
            :class="viewMode === 'list' ? 'bg-indigo-50 text-indigo-600 shadow-sm' : 'text-slate-400 hover:text-slate-600'" 
            class="p-2 rounded-md transition-all duration-200">
            <List class="w-4 h-4" />
          </button>
        </div>

        <button @click="openAdd" class="bg-indigo-600 hover:bg-indigo-700 text-white px-5 py-2.5 rounded-xl text-sm  shadow-lg shadow-indigo-200 flex items-center gap-2 transition-all active:scale-95">
          <Plus class="w-4 h-4" /> 
    ...
    <div class="sticky top-0 z-30 -mx-4 px-4 md:mx-0 md:px-0 mb-6">
      <div class="bg-white/80 backdrop-blur-xl border-y md:border border-slate-200 md:rounded-2xl p-3 shadow-lg shadow-slate-200/40 flex flex-col gap-3">
        
        <div class="flex gap-3">
          <div class="relative flex-1 group">
            <Search class="absolute left-3.5 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-400 group-focus-within:text-indigo-600 transition-colors" />
            <input 
              v-model="searchQuery" 
              type="text" 
        ...
        <div class="flex items-center gap-2 overflow-x-auto no-scrollbar pb-1">
          <div class="flex items-center gap-1.5 pr-4">
            ...
            <button 
              ...
              :class="selectedCategory === cat ? 'bg-indigo-600 text-white border-indigo-600 shadow-md shadow-indigo-200' : 'bg-white text-slate-500 border-slate-200 hover:border-slate-300 hover:bg-slate-50'"
            >
              {{ cat }}
            </button>
          </div>
    ...
    <div v-else-if="filteredProducts.length === 0" class="flex flex-col items-center justify-center py-20 px-4 text-center">
      ...
      <p class="text-slate-500 text-sm max-w-xs mx-auto mb-6">We couldn't find anything matching your search. Try adjusting filters.</p>
      <button @click="searchQuery = ''; selectedCategory = 'All'" class="text-sm  text-indigo-600 hover:text-indigo-700 hover:underline">
        Clear all filters
      </button>
    </div>
      ...
      <div v-if="viewMode === 'grid'" class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3 md:gap-6">
        <div v-for="item in filteredProducts" :key="item.id" 
          ...
          <div class="aspect-square relative overflow-hidden bg-slate-50">
            <img v-if="item.image_url" :src="item.image_url" class="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500" loading="lazy" />
          ...
          <div class="p-3 md:p-4 flex flex-col flex-1">
            <div class="mb-2">
              ...
              <h3 class="font-medium text-slate-900 text-sm md:text-sm leading-tight line-clamp-2">{{ item.name }}</h3>
            </div>
            
            <div class="mt-auto flex items-end justify-between border-t border-slate-50">
          ...
          <div v-for="item in filteredProducts" :key="item.id" 
            ...
            <div class="flex items-center gap-3 md:col-span-5 md:w-full">
              ...
              <div class="flex-1 min-w-0">
                 ...
                 <div class="md:hidden flex items-center gap-2 text-xs text-slate-500">
                   ...
                   <span :class="item.available ? 'text-green-600' : 'text-red-500'">{{ item.available ? 'In Stock' : 'Out' }}</span>
                 </div>
              </div>
            </div>
            ...
            <div class="absolute top-3 right-3 md:static md:col-span-1 md:flex md:justify-end">
              <div class="flex items-center gap-1">
                <button @click.stop="openEdit(item)" class="p-2 md:p-2 text-slate-400 hover:text-indigo-600 hover:bg-indigo-50 rounded-lg transition-all" title="Edit">
                  <Edit2 class="w-5 h-5 md:w-4 md:h-4" />
                </button>
                <button @click.stop="deleteProduct(item.id)" class="hidden md:block p-2 text-slate-400 hover:text-red-600 hover:bg-red-50 rounded-lg transition-all" title="Delete">
