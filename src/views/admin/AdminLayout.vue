...
<template>
  <div class="flex h-screen bg-slate-50 font-sans text-slate-900 overflow-hidden text-xs">
    ...
    >
      <div class="h-16 flex items-center justify-center min-h-16 relative overflow-hidden mt-6">
        <div class="flex items-center justify-start w-full px-4 transition-all duration-300">
           ...
           <div class="flex flex-col justify-center whitespace-nowrap overflow-hidden transition-all duration-500 ease-in-out" :class="textClass">
      ...
      <nav class="flex-1 px-3 space-y-1.5 mt-6 overflow-x-hidden">
        <router-link
          ...
          :class="isActive(item.path) ? 'bg-slate-50 text-slate-900 font-semibold' : 'text-slate-500 hover:bg-slate-50 hover:text-slate-900'"
        >
          <component :is="item.icon" class="w-5 h-5 shrink-0 transition-transform duration-300 ease-out" :class="{ 'text-slate-900': isActive(item.path) }" />
          <span class="whitespace-nowrap overflow-hidden transition-all duration-500 ease-in-out font-medium" :class="textClass">{{ item.name }}</span>
        </router-link>
      </nav>

      <div class="p-3 space-y-1 mb-6">
        <button @click="router.push('/')" class="w-full group relative flex items-center px-3 py-2.5 rounded-xl font-medium text-slate-500 hover:bg-slate-100 hover:text-slate-900 transition-all duration-200">
          <ArrowLeft class="w-5 h-5 shrink-0" />
          <span class="whitespace-nowrap overflow-hidden transition-all duration-500 ease-in-out" :class="textClass">POS Terminal</span>
        </button>
        <button @click="handleLogout" class="w-full group relative flex items-center px-3 py-2.5 rounded-xl font-medium text-red-500 hover:bg-red-50 hover:text-red-600 transition-all duration-200">
          <LogOut class="w-5 h-5 shrink-0" />
          <span class="whitespace-nowrap overflow-hidden transition-all duration-500 ease-in-out" :class="textClass">Sign Out</span>
        </button>
      </div>
    </aside>

    <nav class="lg:hidden fixed bottom-0 left-0 right-0 z-50 flex justify-around items-center bg-white/95 backdrop-blur-xl border-t border-slate-200 shadow-lg mobile-nav-fix">
      ...
      >
        ...
        />
        <span class="text-[9px] font-bold tracking-wide" :class="isActive(item.path) ? 'opacity-100' : 'opacity-75'">
           {{ item.name }}
        </span>
      </router-link>
    ...
    <transition name="slide-up">
      <div v-if="showMobileMenu" class="lg:hidden fixed inset-0 z-[60] flex flex-col justify-end">
        
        <div @click="showMobileMenu = false" class="absolute inset-0 bg-slate-900/40 backdrop-blur-sm transition-opacity"></div>
        
        <div class="bg-white w-full rounded-t-3xl p-6 relative z-10 shadow-2xl pb-safe">
          
          <div class="w-12 h-1.5 bg-slate-200 rounded-full mx-auto mb-6"></div>

          <div class="flex items-center justify-between mb-6">
            <div class="flex items-center gap-3">
              <div class="w-10 h-10 bg-indigo-600 rounded-full flex items-center justify-center text-white font-bold font-preah text-lg">
                {{ userStore.storeName ? userStore.storeName.charAt(0) : 'S' }}
              </div>
              <div>
                <h3 class="font-bold text-base text-slate-900">{{ userStore.storeName || 'Store' }}</h3>
                <p class="text-xs text-slate-500 font-medium uppercase">{{ userStore.role }} Account</p>
              </div>
            </div>
            <button @click="showMobileMenu = false" class="p-2 bg-slate-100 rounded-full text-slate-500 hover:bg-slate-200">
              <XIcon class="w-5 h-5" />
            </button>
          </div>
          ...
          <div class="grid grid-cols-4 gap-4 mb-8">
            ...
            >
              <div 
                ...
                :class="isActive(item.path) ? 'bg-indigo-600 text-white shadow-indigo-200 shadow-lg' : 'bg-slate-50 text-slate-600 hover:bg-slate-100'"
              >
                <component :is="item.icon" class="w-6 h-6" :stroke-width="2" />
              </div>
              <span class="text-[10px] font-semibold text-slate-600 text-center leading-tight">{{ item.name }}</span>
            </router-link>
          </div>
