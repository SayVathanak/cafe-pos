...
<template>
  <div class="flex h-screen bg-white font-sans text-slate-900 overflow-hidden text-xs">
    ...
    >
      <div class="h-16 flex items-center justify-between px-5 border-b border-slate-50 mt-6">
        <div>
          <div class="text-xl font-medium font-khmer text-slate-900">សាយ័ណ្ហកាហ្វេ</div>
          <p class="text-[10px] font-mono text-slate-400 uppercase tracking-widest -mt-0.5">Admin Console</p>
        </div>
        <button @click="isSidebarOpen = false" class="lg:hidden text-slate-400 hover:text-black transition-colors">
          <X class="w-4 h-4" />
        </button>
      </div>

      <nav class="flex-1 px-3 py-6 space-y-1">
        <div class="px-3 mb-2 text-[10px] font-bold text-slate-300 uppercase tracking-widest">Menu</div>
        ...
        >
          <LayoutDashboard class="w-4 h-4 transition-transform group-hover:scale-110 duration-200" />
          <span>Overview</span>
        </router-link>
        ...
        >
          <Coffee class="w-4 h-4 transition-transform group-hover:scale-110 duration-200" />
          <span>Inventory</span>
        </router-link>
        ...
        >
          <Settings class="w-4 h-4 transition-transform group-hover:scale-110 duration-200" />
    ...
    <main class="flex-1 flex flex-col min-w-0 h-full relative bg-slate-50/50">
      ...
      <header class="lg:hidden h-14 bg-white border-b border-slate-100 flex items-center justify-between px-4 shrink-0 shadow-sm z-30">
        <div class="font-bold text-sm tracking-tight">Admin Dashboard</div>
        <button @click="isSidebarOpen = true" class="p-2 -mr-2 text-slate-500 hover:text-black">
          <Menu class="w-5 h-5" />
        </button>
      </header>

      <div class="flex-1 overflow-y-auto p-4 lg:p-8 scroll-smooth scrollbar-hide">
