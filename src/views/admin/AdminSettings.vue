...
const { fetchUsage, checkLimit, currentPlan, usage, limits, planConfigs } =
  usePlanLimits();

const inputClass =
  "w-full px-3 py-2 bg-slate-50 border border-slate-200 rounded-lg text-xs font-medium text-slate-900 focus:outline-none focus:border-slate-400 focus:ring-0 transition-all placeholder:text-slate-300";
...
const formatLimitText = (val) =>
  val === Infinity || val === null ? "Unlimited" : val;
...
// Preview Logic
const previewTotal = 5.0;
const previewTaxVal = computed(() =>
  showTax.value ? (previewTotal * (form.value.tax_rate || 0)) / 100 : 0
);
const previewServiceVal = computed(() =>
  showService.value
    ? (previewTotal * (form.value.service_charge || 0)) / 100
    : 0
);
const finalUsd = computed(
  () => previewTotal + previewTaxVal.value + previewServiceVal.value
);
const finalRiel = computed(
  () =>
    Math.ceil((finalUsd.value * (form.value.exchange_rate || 4000)) / 100) * 100
);
const baseRiel = computed(
  () =>
    Math.ceil((previewTotal * (form.value.exchange_rate || 4000)) / 100) * 100
);
...
const getPlanValidationErrors = (targetPlanId) => {
  ...
  // Check Branches
  if (
    targetPlan.max_branches !== null &&
    usage.value.branches > targetPlan.max_branches
  ) {
    ...
    details: [
      {
        ...
        current: "Next Cycle",
        limit: "",
        action: "Changes apply automatically when your current plan expires.",
      },
      {
        icon: Ban,
        title: "Feature Loss",
        current: "Branding",
        limit: "",
        action: "You will lose custom receipt branding and premium support.",
      },
    ],
...
const executeDowngrade = async (plan) => {
  ...
  if (error) {
    ...
    toast.addToast(
      `Downgrade scheduled. You will switch to ${plan.name} next month.`,
      "success"
    );
  }
};
...
const handleLogoUpload = async (event) => {
  ...
  try {
    const fileName = `${userStore.organizationId}-${Date.now()}.${file.name
      .split(".")
      .pop()}`;
    const { error: upErr } = await supabase.storage
      .from("logos")
      .upload(`store-logos/${fileName}`, file, { upsert: true });
    if (upErr) throw upErr;
    const { data } = supabase.storage
      .from("logos")
      .getPublicUrl(`store-logos/${fileName}`);
...
<template>
  <div class="relative">
    ...
    >
      ...
      <div class="flex items-center gap-2 self-end sm:self-auto">
        ...
        <button
          @click="saveSettings"
          :disabled="saving"
          class="flex items-center gap-2 bg-slate-900 text-white px-5 py-2 rounded-lg text-xs font-bold hover:bg-slate-800 transition-all disabled:opacity-50 shadow-sm"
        >
          <Loader2 v-if="saving" class="w-3.5 h-3.5 animate-spin" /><Save
            v-else
            class="w-3.5 h-3.5"
    ...
    <div class="flex flex-col lg:flex-row gap-10 items-start">
      <div class="flex-1 w-full min-w-0">
        ...
        >
          <div v-if="activeTab === 'identity'" class="space-y-6">
            <div
              class="flex items-start gap-6 bg-white p-6 rounded-xl border border-slate-200 shadow-sm"
            >
              <div
                class="shrink-0 group relative w-24 h-24 rounded-lg border border-slate-100 bg-slate-50 flex items-center justify-center overflow-hidden"
              >
                <img
                  v-if="form.logo_url"
                  :src="form.logo_url"
                ...
                <div
                  v-if="!checkLimit('custom_logo')"
                  class="absolute inset-0 bg-slate-100/90 flex flex-col items-center justify-center text-slate-400"
                >
                  <Lock class="w-4 h-4 mb-1" /><span
                ...
                <label
                  v-else
                  class="absolute inset-0 bg-black/50 opacity-0 group-hover:opacity-100 flex items-center justify-center cursor-pointer transition-opacity text-white"
                  ><Upload class="w-4 h-4" /><input
                    type="file"
                    class="hidden"
              ...
              <div class="flex-1 space-y-4">
                ...
                <div class="flex items-center gap-2">
                  <input
                    type="checkbox"
                    v-model="form.use_logo_header"
                    :disabled="!checkLimit('custom_logo')"
            ...
            <div
              class="bg-white p-6 rounded-xl border border-slate-200 shadow-sm grid grid-cols-1 md:grid-cols-2 gap-4"
            >
              <div class="md:col-span-2">
                <label
                  class="text-[10px] font-bold uppercase text-slate-400 flex items-center gap-1 mb-1.5"
                  ><MapPin class="w-3 h-3" /> Address</label
                ><textarea
              ...
              <div>
                <label
                  class="text-[10px] font-bold uppercase text-slate-400 flex items-center gap-1 mb-1.5"
                  ><Phone class="w-3 h-3" /> Phone</label
                ><input
            ...
            <div
              class="flex items-center justify-center gap-2 text-[10px] text-slate-400"
            >
              <Info class="w-3 h-3" />
              <span
                >These details are used to configure your receipt branding.</span
              >
            </div>
          </div>
          ...
          <div v-if="activeTab === 'billing'" class="space-y-6">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div
                class="col-span-1 md:col-span-2 bg-slate-900 text-white rounded-xl p-6 shadow-md flex items-center justify-between relative overflow-hidden"
              >
                <div
                  class="absolute right-0 top-0 h-full w-1/3 bg-linear-to-l from-white/10 to-transparent pointer-events-none"
                ></div>
                <div class="relative z-10">
                  ...
                  <div class="text-2xl font-bold flex items-center gap-3">
                    {{ limits.name }}
                    <span
                      class="px-2 py-0.5 rounded-md bg-emerald-500/20 border border-emerald-500/50 text-emerald-300 text-[9px] font-bold uppercase tracking-wide"
                      >Active</span
                    >
                  </div>
                </div>
              ...
              <div
                class="p-5 bg-white border border-slate-200 rounded-xl shadow-sm"
              >
                <div class="flex justify-between items-end mb-3">
                  ...
                  ><span class="text-[10px] font-mono text-slate-400"
                    >{{ usage.items }} /
                    {{ displayLimit(limits.max_items) }}</span
                  >
                </div>
                <div
              ...
              >
                <div class="flex justify-between items-end mb-3">
                  ...
                  ><span class="text-[10px] font-mono text-slate-400"
                    >{{ usage.orders_month }} /
                    {{ displayLimit(limits.max_orders) }}</span
                  >
                </div>
                <div
                  class="h-2 w-full bg-slate-100 rounded-full overflow-hidden"
            ...
            <section>
              <div class="flex items-center justify-between mb-4 mt-2">
                ...
                <div
                  v-if="pendingRequest"
                  class="flex items-center gap-2 px-3 py-1.5 bg-amber-50 text-amber-600 rounded-lg border border-amber-200"
                >
                  <Clock class="w-3.5 h-3.5" /><span
                    class="text-[10px] font-bold"
                    >Payment Pending Review</span
                  >
                </div>
              </div>
              <div class="grid grid-cols-1 md:grid-cols-3 gap-3">
                <div
                  ...
                  :key="plan.id"
                  class="relative p-5 rounded-xl border transition-all flex flex-col justify-between min-h-35"
                  :class="
                    currentPlan === plan.id
                ...
                >
                  <div
                    ...
                    class="absolute top-3 right-3 text-emerald-600"
                  >
                  ...
                  <div>
                    ...
                    <div class="text-2xl font-bold text-slate-900">
                      {{ plan.price > 0 ? "$" + plan.price : "Free"
                        ...
                        v-if="plan.price > 0"
                        class="text-[10px] font-medium text-slate-400 ml-0.5"
                        >/mo</span
                      >
                    </div>
                  </div>
                  <div class="my-4 space-y-2">
                    ...
                    >
                      <Store class="w-3.5 h-3.5 text-slate-400" /><span
                        >{{ formatLimitText(plan.max_branches) }} Branch{{
                          plan.max_branches !== 1 ? "es" : ""
                        }}</span
                      >
                    </div>
                    <div
                      class="flex items-center gap-2 text-[10px] text-slate-600"
                    >
                      <List class="w-3.5 h-3.5 text-slate-400" /><span
                        >{{ formatLimitText(plan.max_items) }} Menu Items</span
                      >
                    </div>
                    <div
                      class="flex items-center gap-2 text-[10px] text-slate-600"
                    >
                      <ShoppingBag class="w-3.5 h-3.5 text-slate-400" /><span
                        >{{ formatLimitText(plan.max_orders) }} Orders/mo</span
                      >
                    </div>
                    <div
                      class="flex items-center gap-2 text-[10px] text-slate-600"
                    >
                      <Image class="w-3.5 h-3.5 text-slate-400" /><span>{{
                        plan.allow_custom_logo
                          ? "Custom Branding"
                          : "Standard Branding"
                      }}</span>
                    </div>
                  ...
                  <div class="mt-auto pt-3 border-t border-slate-100">
                    <button
                      v-if="currentPlan !== plan.id && plan.price > 0"
                      @click="openPaymentModal(plan.id, plan.price)"
                      :disabled="!!pendingRequest"
                      class="w-full py-2 bg-black text-white rounded-lg text-xs font-bold hover:bg-slate-800 disabled:opacity-50 disabled:cursor-not-allowed"
                    >
                      {{ pendingRequest ? "Pending..." : "Switch Plan" }}
                    </button>
                    <button
                      v-else-if="currentPlan !== plan.id && plan.price === 0"
                      @click="handleDowngradeClick(plan)"
                      class="w-full py-2 border border-slate-200 text-slate-600 rounded-lg text-xs font-bold hover:bg-slate-50 flex items-center justify-center gap-2"
                    >
                      <ArrowDown class="w-3 h-3" /> Downgrade
                    </button>
                    <div v-else-if="currentPlan === plan.id">
                      <div v-if="plan.price > 0" class="flex flex-col gap-2">
                        <div
                          class="text-center text-[10px] font-bold text-emerald-600 bg-emerald-100/50 py-1 rounded"
                        >
                          Current Plan
                        </div>
                        <button
                          @click="openPaymentModal(plan.id, plan.price)"
                          :disabled="!!pendingRequest"
                          class="w-full py-2 border border-slate-900 text-slate-900 rounded-lg text-xs font-bold hover:bg-slate-50 disabled:opacity-50"
                        >
                          {{ pendingRequest ? "Pending..." : "Renew / Extend" }}
                        </button>
                      </div>
                      <div
                        v-else
                        class="text-center text-[10px] font-bold text-slate-400 py-2"
                      >
                        Active Free Plan
                      </div>
                    </div>
            ...
            >
              ...
              >
                ...
                ><input
                  ...
                  type="number"
                  class="w-20 text-right text-lg font-bold bg-transparent border-none p-0 focus:ring-0"
            ...
            >
              <div class="flex items-center justify-between">
                <div class="flex items-center gap-4">
                  ...
                  <div>
                    <div class="text-sm font-bold text-slate-900">VAT Tax</div>
                    <div class="text-[10px] text-slate-500">
                      Percentage added to subtotal
                    </div>
                  </div>
                </div>
                <div class="flex items-center gap-3">
                  <input
                    ...
                    type="number"
                    class="w-16 text-center text-sm font-bold border border-slate-200 rounded-lg py-1.5 focus:border-blue-500 focus:ring-1 focus:ring-blue-500"
                  /><button
                    @click="showTax = !showTax"
                    class="w-10 h-6 rounded-full transition-colors relative"
                    :class="showTax ? 'bg-blue-600' : 'bg-slate-200'"
                  >
                    <span
                      class="absolute top-1 left-1 w-4 h-4 bg-white rounded-full transition-transform shadow-sm"
                      :class="showTax ? 'translate-x-4' : ''"
                    ></span>
                  </button>
                </div>
              </div>
              ...
              <div class="flex items-center justify-between">
                ...
                <div class="flex items-center gap-3">
                  <input
                    ...
                    type="number"
                    class="w-16 text-center text-sm font-bold border border-slate-200 rounded-lg py-1.5 focus:border-emerald-500 focus:ring-1 focus:ring-emerald-500"
                  /><button
                    @click="showService = !showService"
                    class="w-10 h-6 rounded-full transition-colors relative"
                    :class="showService ? 'bg-emerald-600' : 'bg-slate-200'"
                  >
                    <span
                      class="absolute top-1 left-1 w-4 h-4 bg-white rounded-full transition-transform shadow-sm"
                      :class="showService ? 'translate-x-4' : ''"
                    ></span>
                  </button>
                </div>
              </div>
          ...
          <div v-if="activeTab === 'system'" class="space-y-6">
            ...
            >
              <label
                class="text-[10px] font-bold uppercase text-slate-400 mb-4 block"
                >Default Paper Size</label
              >
              <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <button
                  @click="form.paper_size = '58mm'"
                  class="px-5 py-4 rounded-xl border-2 text-left transition-all"
                  :class="
                ...
                >
                  ...
                  <div class="text-[10px] text-slate-500">
                    Standard for portable Bluetooth printers.
                  </div></button
                ><button
                ...
                >
                  ...
                  <div class="text-[10px] text-slate-500">
                    Standard for kitchen & countertop printers.
                  </div>
                </button>
              </div>
            </div>
      ...
      <div class="hidden md:block w-72 lg:w-80 shrink-0 relative">
        <div class="sticky top-0">
          <div
            class="text-[10px] font-bold uppercase tracking-widest text-slate-400 mb-4 flex items-center gap-2"
          >
            <Monitor class="w-3 h-3" /> Live Receipt Preview
          </div>

          <div class="relative w-full drop-shadow-xl transition-transform">
            <div
            ...
            <div
              class="bg-white p-6 pt-6 pb-8 text-[10px] font-mono leading-tight relative min-h-90"
            >
              ...
              <div class="flex justify-between mb-1">
                <span>Order #:</span>
                <span class="font-bold text-slate-900">#001</span>
              </div>
              ...
              <div class="space-y-3 mb-4">
                ...
                <div
                  class="text-[9px] text-slate-500 font-sans italic -mt-2 ml-2"
                >
                  • Oat Milk, Less Sugar
                </div>
              ...
              >
                ...
                >
                  <span>Subtotal</span>
                  <span>{{ baseRiel.toLocaleString() }} ៛</span>
                </div>
                ...
                >
                  ...
                  <span
                    >{{
                      (baseRiel * (form.tax_rate / 100)).toLocaleString()
                    }}
                    ៛</span
                  >
                </div>
              ...
              <div class="text-center space-y-4">
                ...
                >
                  ...
                  <p
                    class="font-bold text-slate-900 tracking-wider text-[10px]"
                  >
                    {{ form.wifi_pass }}
                  </p>
                </div>
    ...
    >
      <div
        class="bg-white w-full max-w-sm rounded-xl shadow-2xl p-6 relative overflow-hidden"
      >
        <div class="flex justify-center mb-4">
          <div
            v-if="actionModal.mode === 'error'"
            class="w-12 h-12 bg-red-100 rounded-full flex items-center justify-center text-red-600 animate-in zoom-in-50 duration-300"
          >
            <Ban class="w-6 h-6" />
          </div>
          <div
            v-else
            class="w-12 h-12 bg-amber-100 rounded-full flex items-center justify-center text-amber-600 animate-in zoom-in-50 duration-300"
          >
            <AlertTriangle class="w-6 h-6" />
          </div>
        </div>
        <h2 class="text-lg font-bold text-center mb-2 text-slate-900">
          {{ actionModal.title }}
        </h2>
        ...
        <div
          v-if="actionModal.details && actionModal.details.length"
          class="bg-slate-50 rounded-lg p-3 border border-slate-100 mb-6 space-y-3"
        >
          <div
            v-for="(detail, i) in actionModal.details"
            :key="i"
            class="flex gap-3 text-[11px] items-start"
          >
            <div
              class="mt-0.5 p-1 bg-white border border-slate-200 rounded shrink-0"
            >
              <component :is="detail.icon" class="w-3 h-3 text-slate-500" />
            </div>
            <div class="flex-1">
              <div class="flex justify-between items-center mb-0.5">
                ...
                ><span
                  v-if="detail.current"
                  class="text-[9px] bg-red-100 text-red-600 px-1.5 py-0.5 rounded font-mono"
                  >{{ detail.current }}</span
                >
              </div>
        ...
        <div class="flex gap-2">
          <button
            @click="actionModal.secondaryAction()"
            class="flex-1 py-2.5 border border-slate-200 rounded-lg font-bold text-xs hover:bg-slate-50 transition-colors text-slate-600"
          >
            {{ actionModal.secondaryLabel }}</button
          ><button
            @click="actionModal.primaryAction()"
    ...
    >
      <div class="bg-white w-full max-w-sm rounded-xl p-6 shadow-2xl">
        <h2 class="text-lg font-bold mb-1">
          {{
            currentPlan === paymentPlan?.id ? "Renew / Extend" : "Upgrade Plan"
          }}
        </h2>
        <p class="text-xs text-slate-500 mb-4">
          Transfer
          <span class="font-bold text-black">${{ paymentPlan?.price }}</span> to
          the account below.
        </p>
        <div
          class="bg-slate-50 p-4 rounded-lg border border-slate-200 mb-4 space-y-2"
        >
        ...
        <div class="mb-4">
          <label
            ...
            ><div
              v-if="!paymentPreview"
              class="border-2 border-dashed border-slate-300 rounded-xl h-32 flex flex-col items-center justify-center gap-2 group-hover:border-slate-400 group-hover:bg-slate-50 transition-colors"
            >
              <FileText class="w-6 h-6 text-slate-400" /><span
            ...
            <img
              v-else
              :src="paymentPreview"
              class="w-full h-32 object-cover rounded-xl border border-slate-200" /><input
              type="file"
              class="hidden"
        ...
        <div class="flex gap-2">
          <button
            ...
            class="flex-1 py-2.5 border rounded-lg font-bold text-xs hover:bg-slate-50"
          >
            Cancel</button
          ><button
            @click="submitPayment"
