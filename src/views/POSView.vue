...
const confirmAddToCart = (drinkWithOptions) => {
  ...
  showOptions.value = false; // Just close, no toast needed
};
...
<template>
  <div class="flex h-screen bg-slate-50 text-slate-900 font-sans antialiased overflow-hidden">
    ...
    <main class="flex-1 flex flex-col min-w-0 relative">
      <header class="h-20 bg-white/80 backdrop-blur-lg px-6 flex items-center justify-between sticky top-0 z-30 shrink-0 border-b border-slate-200/50">
        <div class="flex items-center gap-6">
          <button @click="router.push('/admin')" class="text-left group">
            <div class="flex flex-col items-start">
              <div class="text-xl font-medium tracking-tight font-preahvihear group-hover:text-slate-600 transition-colors">សាយ័ណ្ហកាហ្វេ</div>
        ...
        <div class="flex items-center gap-4">
          ...
          <button @click="isCartOpen = true" class="lg:hidden relative p-2 bg-slate-100 rounded-full shrink-0 active:scale-95 transition-transform">
      ...
      <div class="px-6 py-4 flex gap-3 overflow-x-auto scrollbar-hide shrink-0">
        <button v-for="cat in categories" :key="cat" @click="selectedCategory = cat" class="border-none rounded-full px-5 py-2 text-xs font-medium uppercase tracking-wider transition-all whitespace-nowrap flex items-center gap-1.5 active:scale-95"
      ...
      <section class="flex-1 overflow-y-auto scroll-smooth p-6 pt-0">
        ...
        <div v-else class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4 pb-32 lg:pb-0 auto-rows-fr">
    ...
    <aside 
      class="fixed inset-y-0 right-0 z-50 w-full sm:w-100 bg-white border-l border-slate-200 transition-all duration-500 lg:static lg:translate-x-0 lg:shadow-none shrink-0" 
      :class="isCartOpen ? 'translate-x-0 shadow-2xl' : 'translate-x-full shadow-none'"
    >
    ...
    <button 
      ...
      class="lg:hidden fixed left-4 right-4 z-50 bg-black text-white px-6 py-4 flex items-center justify-between text-sm font-medium rounded-2xl shadow-2xl shadow-black/25 active:scale-95 transition-all mobile-float-button"
    >
      <div class="flex items-center gap-3">
        <span class="bg-white text-black rounded-full w-6 h-6 flex items-center justify-center text-xs font-black">
          {{ cartStore.items.length }}
        </span>
        <span class="uppercase tracking-widest font-medium">View Order</span>
      </div>
