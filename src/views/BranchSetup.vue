...
const handleRegister = async () => {
  ...
  // 1. Fetch the organization_id from the store first
  ...
  // 2. Pass organization_id into the signup metadata
...
<template>
  <div class="min-h-screen bg-slate-50 flex items-center justify-center p-4">
    <div
      class="max-w-md w-full bg-white rounded-2xl shadow-xl overflow-hidden border border-slate-100 p-8"
    >
      <div class="text-center mb-8">
        <div
          class="w-12 h-12 bg-indigo-50 rounded-xl flex items-center justify-center mx-auto mb-4 text-indigo-600"
        >
          <Store class="w-6 h-6" />
        </div>
        <h1 class="text-xl font-bold text-slate-900">Activate Branch Login</h1>
        <p v-if="!errorMsg" class="text-sm text-slate-500 mt-2">
          Setting up for:
          <strong class="text-slate-800">{{ storeName }}</strong>
        </p>
      </div>
      ...
      <div
        v-else-if="errorMsg"
        class="p-4 bg-red-50 text-red-600 text-sm rounded-xl text-center mb-4"
      >
        {{ errorMsg }}
      </div>
      ...
      <div v-else class="space-y-4">
        <div>
          ...
          <input
            v-model="form.email"
            type="email"
            class="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl text-sm focus:outline-none focus:border-indigo-600 transition-colors"
            placeholder="branch@sayon.com"
          />
        </div>
