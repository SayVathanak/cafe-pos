...
<template>
  <div
    class="min-h-screen flex items-center justify-center bg-zinc-50 font-sans text-zinc-900 p-6"
  >
    <div class="w-full max-w-md bg-white border border-zinc-200 p-10 relative">
      ...
      <div class="mb-10">
        <div
          class="w-10 h-10 bg-black text-white flex items-center justify-center mb-6"
        >
          <span class="font-bold text-lg">S</span>
        </div>
        <h1 class="text-2xl font-bold tracking-tight uppercase mb-2">
          Admin Access
        </h1>
      ...
      <form @submit.prevent="handleLogin" class="space-y-8">
        <div
          v-if="errorMsg"
          class="bg-red-50 border border-red-100 p-3 flex items-start gap-2"
        >
          <div
            class="text-red-600 text-xs font-bold uppercase tracking-wide leading-relaxed"
          >
            Error: {{ errorMsg }}
          </div>
        </div>
        ...
        <div class="group">
          <label
            class="block text-[10px] font-bold uppercase tracking-widest text-zinc-400 mb-2 group-focus-within:text-black transition-colors"
          >
            Email Address
          </label>
          <div
            class="flex items-center border-b border-zinc-200 group-focus-within:border-black transition-colors py-2"
          >
            <Mail
              class="w-4 h-4 text-zinc-400 group-focus-within:text-black mr-3"
            />
            <input
              v-model="email"
              type="email"
